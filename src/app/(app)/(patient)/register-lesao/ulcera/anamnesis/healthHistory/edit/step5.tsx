...
export default function UlceraHealthHistoryEditStep5() {
  ...
  const handleNext = (data: UlceraHealthHistoryProps) => {
    if (data.compression_stockings_use && data.compression_stockings_use.length > 0 && notEmpty) {
      ...
      router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/edit/step6')
    } else {
      return;
    }
  }
    ...
    >
      ...
      <View className="gap-4 mt-4 px-8 w-full justify-start mb-4 flex-row">
        <Button title="Voltar" 
          ...
          onPress={()=> router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/edit/step4')} 
