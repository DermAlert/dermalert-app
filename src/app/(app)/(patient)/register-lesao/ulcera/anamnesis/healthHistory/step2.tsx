...
export default function UlceraHealthHistoryStep2() {
  ...
  const cancerTypeValue = useWatch({ control, name: "diabetes" });


  const handleNext = (data: UlceraHealthHistoryProps) => {
    if (data.diabetes && data.diabetes.length > 0 && notEmpty) {
      console.log(data);
      updateUlceraHealthHistoryData(data);
      router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/step3')
    } else {
      return;
    }
  }
  ...
  return (
    ...
    >
      ...
      <ScrollView className="px-8 w-full flex-1">
        ...
        <Text allowFontScaling={false} className="text-base text-neutral-800 mt-4 mb-8">O paciente possui diagnóstico de diabetes mellitus?</Text>
          ...
          render={({ field: { onChange, value } }) => (
            <View className="gap-3">
              <RadioButton label="Sim" value="YES" checked={value === 'YES'} onPress={() => {
                ...
                onChange(newValue);
                setNotEmpty(true);
                updateUlceraHealthHistoryData({ diabetes: newValue });
                router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/step3')
              }} />
      ...
      <View className="gap-4 mt-4 px-8 w-full justify-start mb-4 flex-row">
        <Button title="Voltar" 
          ...
          onPress={()=> router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/step1')} 
