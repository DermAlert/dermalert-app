...
export default function UlceraHealthHistoryEditStep4() {
  ...
  const cancerTypeValue = useWatch({ control, name: "chronic_venous_insufficiency" });


  const handleNext = (data: UlceraHealthHistoryProps) => {
    if (data.chronic_venous_insufficiency && data.chronic_venous_insufficiency.length > 0 && notEmpty) {
      console.log(data);
      updateUlceraHealthHistoryData(data);
      router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/edit/step5')
    } else {
      return;
    }
  }
  ...
  return (
    ...
    >
      ...
      <ScrollView className="px-8 w-full flex-1">
        ...
        <Text allowFontScaling={false} className="text-base text-neutral-800 mt-4 mb-8">O paciente já foi diagnosticado(a) com insuficiência venosa crônica?</Text>
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <View className="gap-3">
              <RadioButton label="Sim" value="YES" checked={value === 'YES'} onPress={() => {
                ...
                onChange(newValue);
                setNotEmpty(true);
                updateUlceraHealthHistoryData({ chronic_venous_insufficiency: newValue });
                router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/edit/step5')
              }} />
          ...
          name="chronic_venous_insufficiency"
        /> 
      ...
      <View className="gap-4 mt-4 px-8 w-full justify-start mb-4 flex-row">
        <Button title="Voltar" 
          ...
          onPress={()=> router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/edit/step3')} 
