...
export default function UlceraHealthHistoryStep3() {
  ...
  const handleNext = (data: UlceraHealthHistoryProps) => {
    if (data.deep_vein_thrombosis && data.deep_vein_thrombosis.length > 0 && notEmpty) {
      ...
      router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/step4')
    } else {
      return;
    }
  }
  ...
  return (
    ...
    >
      ...
      <ScrollView className="px-8 w-full flex-1">
        ...
        <Text className="text-base text-neutral-800 mt-4 mb-8">O paciente já teve trombose venosa profunda (TVP)?</Text>
      ...
      <View className="gap-4 mt-4 px-8 w-full justify-start mb-4 flex-row">
        <Button title="Voltar" 
          ...
          onPress={()=> router.push('/(app)/(patient)/register-lesao/ulcera/anamnesis/healthHistory/step2')} 
