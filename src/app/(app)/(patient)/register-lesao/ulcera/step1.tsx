...
export default function RegisterLesaoUlceraStep1() {
  ...
  const { control, handleSubmit } = useForm<LesaoUlceraProps>(
    {
      defaultValues: {
        lesion_local: registerLesionUlceraData.lesion_local
      }
    }
  );
  ...
  useFocusEffect(
    useCallback(() => {
      if(registerLesionUlceraData.lesion_local && registerLesionUlceraData.lesion_local !== undefined) {
        setNotEmpty(true);
        setOpenAccordion(registerLesionUlceraData.lesion_local?.body_part || null);
      }
    }, [])
  );
