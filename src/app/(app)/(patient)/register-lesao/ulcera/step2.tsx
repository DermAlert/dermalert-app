...
export default function RegisterLesaoUlceraStep2() {
  ...
  const { setRegisterLesionUlceraData, updateRegisterLesionUlceraData, imagesUlcera, setImagesUlcera } = useRegisterLesionUlceraForm();

  const { lesionId } = useLesionId();
  const { lesionType, setLesionType } = useLesionType();
  
  const handleNext = (data: LesaoUlceraProps) => {
    if (data.lesion_images && data.lesion_images.length > 0 || imagesUlcera.length > 0) {
      console.log(data);
    ...
    >
      ...
      <ScrollView className="px-8 w-full flex-1">
        ...
        <View className="flex-row flex-wrap gap-4 mt-8">
          ...
          {imagesUlcera.map(item => (
            <PhotoCard key={item} image={item} isDeletable isUlcera />
          ))}
        ...
        (
          <View className="px-8 w-full justify-start mb-4">
            <Button 
              title="Próximo" 
              iconRight 
              icon={<ArrowRightIcon size={24} color={`${imagesUlcera.length > 0 ? 'white' : '#D4D6DF'}`} />} 
              onPress={()=> handleNext({ lesion_images: imagesUlcera })}
              activeOpacity={imagesUlcera.length > 0 ? 0.2 : 1}
              disabled={imagesUlcera.length > 0}
            />
          </View>
        ...
        (
          <View className="gap-4 mt-4 px-8 w-full justify-start mb-4 flex-row">
            ...
            <Button 
              title="Próximo" 
              iconRight 
              icon={<ArrowRightIcon size={24} color={`${imagesUlcera.length > 0 ? 'white' : '#B3B3B3'}`} />}
              style={{ flexGrow: 1, width: '47%' }} 
              onPress={()=> handleNext({ lesion_images: imagesUlcera })} 
              activeOpacity={imagesUlcera.length > 0 ? 0.2 : 1}
              disabled={imagesUlcera.length > 0}
