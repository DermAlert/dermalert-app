...
export default function RegisterLesaoOncodermatoStep6() {
  ...
  const lesionLocalValue = useWatch({ control, name: "diameter" });


  

  const handleNext = (data: LesaoOncodermatoProps) => {
    if (data.diameter && data.diameter.length > 0 && notEmpty) {
      console.log(data);
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <View className="gap-3">
              <RadioButton label="Menor que 6 mm (menor que uma borracha de lápis)" value="under_6mm" checked={value === 'under_6mm'} onPress={() => {
                const newValue = "under_6mm";
                onChange(newValue);
                setNotEmpty(true);
                updateRegisterLesionData({ diameter: newValue });
                router.push('/(app)/(patient)/register-lesao/oncodermato/step7')
              }} />
              <RadioButton label="Maior ou igual a 6 mm." value="over_or_equal_6mm" checked={value === 'over_or_equal_6mm'} onPress={() => {
                const newValue = "over_or_equal_6mm";
                onChange(newValue);
                setNotEmpty(true);
                updateRegisterLesionData({ diameter: newValue });
                router.push('/(app)/(patient)/register-lesao/oncodermato/step7')
